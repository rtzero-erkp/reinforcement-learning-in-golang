...
func TestEpsilonGreed0(t *testing.T) {
	Convey("TestEpsilonGreed0", t, func() {
		...
		var agent = NewEpsilonGreed(epsilon, mesh)
...
func TestEpsilonGreed1(t *testing.T) {
	Convey("TestEpsilonGreed1", t, func() {
		const (
			xRange     = 2.4
			thetaRange = 12
			epsilon    = 0.5
		...
		var (
			...
			agent = NewEpsilonGreed(epsilon, mesh)
			res   = &envs.Result{}
		)
...
func TestEpsilonGreed2(t *testing.T) {
	Convey("TestEpsilonGreed2", t, func() {
		const (
			xRange     = 2.4
			thetaRange = 12
			epsilon    = 0.5
